...
+ [新增] 人物影棚支持固定人脸预设、换装拍摄、严格仿拍、批量变体、逐节点模型选择及付费站账号云同步。
+ [修复] Gemini 原生与 GPT Image 2 请求按模型协议传递 4K 画幅参数，并隔离影棚账号本地数据与跨端删除状态。
+ [修复] 人物影棚恢复刷新后遗留的进行中任务，增加稳定任务序号，记住最高 20 路并发设置，并阻止未明确登录过的浏览器静默继承付费站账号。
+ [修复] 工作流参考图只进入 LLM 规划节点，最终生图仅接收人物身份图，避免仿拍图人脸污染预设身份。
